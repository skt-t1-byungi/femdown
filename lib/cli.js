...
async function main () {
    ...
    const crawler = await withLogOrExit('Attempting to login..', Crawler.login())
    const courseIds = isAll
        ? await withLogOrExit('Scanning Course ID ..', crawler.getCourseIds())
        : input.map(s => s.match(COURSE_ID_RE)[1])

    const limit = pLimit(3)
    const limitRun = runner => limit(() => pRetry(runner, { retries: 5, interval: 1000 }))
    ...
    for (const id of courseIds) {
        ...
        await Promise.all(infos.map(info => {
            ...
            return Promise.all([
                ...
                info.vtt && limitRun(() => downFile(info.vtt, baseDest + '.vtt'))
            ])
        }))
 ...
 * @returns {Promise<T>}
 */
async function withLogOrExit (msg, promise) {
