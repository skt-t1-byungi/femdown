...
module.exports = class Crawler {
    ...
    async getLessonInfos (courseId) {
        const json = await this._request({
            url: `https://api.frontendmasters.com/v1/kabuki/courses/${courseId}`,
            parse: 'json'
        })
        if (!json) throw new Error('Check that "courseId" is correct.')
        if (json.message) throw new Error(json.message)
        const { lessonData, datePublished, hasWebVTT } = json
